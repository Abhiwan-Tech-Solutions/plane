...
class S3Storage(S3Boto3Storage):
    ...
    def __init__(self, request=None):
        ...
        if os.environ.get("USE_MINIO") == "1":
            ...
            # Use MINIO_URL if set (no-proxy setup), otherwise fall back to request host (proxy setup)
            minio_url = os.environ.get("MINIO_URL")
            if minio_url:
                resolved_endpoint = minio_url
            else:
                resolved_endpoint = (f"{endpoint_protocol}://{request.get_host()}" if request else self.aws_s3_endpoint_url)
